  ...
  for {set i 1} {$i<=$restr(num)} {incr i} {
    ...
    switch $restr($i.type) {
      ...
      ADAM {
        ...
        if {![info exists restr($i.tmin)]} {set restr($i.tmin) 1}
        ...
        restr_AddAdamOpt $restr($i.address) $restr($i.a) $restr($i.b1) $restr($i.b2) $restr($i.e) $restr($i.decay) $kT $restr($i.tmin)
      }
